#[test_only]
module sweem_adapters_stsui::stsui_tests;

use sui::test_scenario as ts;
use std::string;
use std::unit_test::destroy;

use sweem_adapters_stsui::stsui::{
    Self as stsui_adapter,
    compute_yield_fee,
};
...
fun fee_zero_when_no_yield() {
    ...
    // gross < principal (mint+redeem fees ate into it) -> no yield -> no fee
...
fun fee_large_values_no_overflow() {
    // ~10 SUI yield (1e10 mist) at 1000 bps -> 1e9
    assert!(compute_yield_fee(110_000_000_000, 100_000_000_000, 1_000) == 1_000_000_000, 0);
}

// ---- registry approval gate ----

// Registry exists but `stsui` is NOT added -> is_approved false -> abort.
// Mirrors the FIRST assert of vault_invest/withdraw, which fires before any
// live AlphaFi object is touched.
#[test]
#[expected_failure(abort_code = stsui_adapter::EProtocolNotApproved)]
fun aborts_when_not_approved() {
    ...
    // Aborts here (stsui not approved) before any live AlphaFi object is read.
    stsui_adapter::assert_approved_for_test(&registry);
...
#[test]
#[expected_failure(abort_code = stsui_adapter::ENotOwner)]
fun aborts_when_not_owner() {
    let mut sc = ts::begin(OWNER);
    registry::init_for_testing(ts::ctx(&mut sc));
    ts::next_tx(&mut sc, OWNER);

    // Approve `stsui` (yield_type 2) so the approval gate passes.
    let mut registry = ts::take_shared<ProtocolRegistry>(&sc);
    let ac = ts::take_shared<AccessControl<REGISTRY>>(&sc);
    registry::add_protocol(
        &mut registry, &ac, string::utf8(b"stsui"), @0x1, 2, ts::ctx(&mut sc),
    );
    ...
    stsui_adapter::assert_owner_for_test(&registry, &vault, ts::ctx(&mut sc));
