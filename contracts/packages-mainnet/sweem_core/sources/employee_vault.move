...
use sui::event;
use sweem_registry::registry::ProtocolRegistry;
...
// Owner-gated mutable UID — prevents arbitrary external packages from attaching
// junk dynamic fields to a transferred/owned vault object.
public fun vault_uid_mut(vault: &mut EmployeeVault, ctx: &TxContext): &mut UID {
    assert!(vault.owner == ctx.sender(), EVaultNotOwner);
    &mut vault.id
}

// Registry-gated mutable UID — TokenBucket carries no owner field, so the bucket's
// owner check is enforced by callers via the parent vault. Requiring a valid
// ProtocolRegistry proves the caller operates within the official system.
public fun bucket_uid_mut<T>(bucket: &mut TokenBucket<T>, _registry: &ProtocolRegistry): &mut UID {
    &mut bucket.id
}

// Allow adapters to extract balance from a bucket for yield investment
public fun split_bucket_for_invest<T>(bucket: &mut TokenBucket<T>, amount: u64): Balance<T> {
    assert!(bucket.balance.value() >= amount, EInsufficientBucketBal);
    bucket.balance.split(amount)
}

// Allow adapters to return yield proceeds back into a bucket
public fun merge_bucket_from_yield<T>(bucket: &mut TokenBucket<T>, bal: Balance<T>) {
    bucket.balance.join(bal);
}
