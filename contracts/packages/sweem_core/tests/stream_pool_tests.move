...
fun create_pool_sets_org() {
    ...
    let pool = stream_pool::create_pool<USDC>(ctx);
    // dummy() sender is @0x0
...
fun deposit_creates_stream_and_credits_balance() {
    ...
    stream_pool::deposit(
        ...
        vector[employee], vector[100u128], vector[1u64],
        &clock, scenario.ctx(),
    );
...
fun deposit_deducts_fee() {
    ...
    // 1% fee → net = 990_000
    assert_eq!(stream_pool::balance_value(&pool), 990_000);
    clock::destroy_for_testing(clock);
    destroy(pool);
...
fun topup_extends_balance() {
    ...
    let p1 = coin::mint_for_testing<USDC>(5_000_000, scenario.ctx());
    stream_pool::deposit(&mut pool, &config, p1, vector[employee], vector[100u128], vector[1u64], &clock, scenario.ctx());
    let p2 = coin::mint_for_testing<USDC>(3_000_000, scenario.ctx());
    stream_pool::topup(&mut pool, &config, p2, scenario.ctx());

    assert_eq!(stream_pool::balance_value(&pool), 8_000_000);
    clock::destroy_for_testing(clock);
    destroy(pool);
    destroy(config);
    scenario.end();
...
fun employee_can_claim_after_accrual() {
    ...
    let payment = coin::mint_for_testing<USDC>(expected_claim + 1_000_000, scenario.ctx());
...
fun non_org_cannot_pause() {
    ...
    let payment = coin::mint_for_testing<USDC>(1_000_000_000, scenario.ctx());
    let mut clock = clock::create_for_testing(scenario.ctx());
    stream_pool::deposit(&mut pool, &config, payment, vector[employee], vector[100u128], vector[1u64], &clock, scenario.ctx());
    clock::increment_for_testing(&mut clock, 100);
    scenario.next_tx(intruder);
    stream_pool::pause_stream(&mut pool, employee, &clock, scenario.ctx());
    abort 0
...
fun claimable_amount_returns_zero_for_unknown() {
    ...
    let mut scenario = test_scenario::begin(org);
    let config = registry::create_test_config(scenario.ctx());
    let pool = stream_pool::create_pool<USDC>(scenario.ctx());
    let clock = clock::create_for_testing(scenario.ctx());
